...
class invoicr extends FPDF_rotation 
{
	...
	*******************************************************************************/
	...
	function addItem($item,$description,$quantity,$perAmt,$price,$discount=0,$total)
	{
		$p['item'] 			= $item;
	...
	*******************************************************************************/
	function Header()
	{
		...
		if(!isset($this->productsEnded)) 
		{
			...
			$this->Cell($width_other,10,iconv("UTF-8", "ISO-8859-1",strtoupper($this->l['area'])),0,0,'C',0);
			$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
			// $this->Cell($width_other,10,iconv("UTF-8", "ISO-8859-1",strtoupper($this->l['vat'])),0,0,'C',0);
			$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
			$this->Cell($width_other,10,iconv("UTF-8", "ISO-8859-1",strtoupper($this->l['rate'])),0,0,'C',0);
			$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
			$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
	...
	{	
		...
		if($this->items) {
			foreach($this->items as $item) 
			{
				...
				// $this->Cell($width_other,$cHeight,$item['area'],0,0,'L',1);
				...
				$this->Cell($width_other,$cHeight,$item['perRate'],0,0,'C',1);
