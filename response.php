...
if ($action == 'create_invoice'){
	...
	foreach($_POST['invoice_product'] as $key => $value) {
		...
		$item_total_price = $_POST['invoice_product_total_price'][$key];
	...
	if($mysqli -> multi_query($query)){
		...
		//Set from
		$invoice->setFrom(array(COMPANY_NAME,COMPANY_ADDRESS_1,COMPANY_ADDRESS_2,COMPANY_ADDRESS_3,COMPANY_CITY,'',COMPANY_COUNTY,COMPANY_POSTCODE,COMPANY_NUMBER,COMPANY_VAT));
		//Set to
		...
		foreach($_POST['invoice_product'] as $key => $value) {
		    ...
		    $invoice->addItem($item_product, '', $item_qty, $item_price, $item_total_price, $item_discount, $item_subtotal);
		}
		//Add totals
...
if($action == 'update_product') {
	...
	$product_area = $_POST['product_area']; // product price
	...
	$stmt->bind_param(
		'sssss',
		$product_name,$product_desc,$product_area,$product_price,$getID
	);
...
if($action == 'update_invoice') {
	...
	if($mysqli -> multi_query($query)) {
		...
		$invoice->setDue($invoice_due_date);
		//Set from
		$invoice->setFrom(array(COMPANY_NAME,COMPANY_ADDRESS_1,COMPANY_ADDRESS_2,COMPANY_ADDRESS_3,COMPANY_CITY,COMPANY_COUNTY,COMPANY_NUMBER,COMPANY_VAT));
		//Set to
		$invoice->setTo(array($customer_name,$customer_address_1,$customer_address_2,$customer_town,$customer_county,$customer_postcode,"Phone: ".$customer_phone));
		//Ship to
		// $invoice->shipTo(array($customer_name_ship,$customer_address_1_ship,$customer_address_2_ship,$customer_town_ship,$customer_county_ship,$customer_postcode_ship,''));
...
if($action == 'add_product') {
	...
	/* Bind parameters. TYpes: s = string, i = integer, d = double,  b = blob */
	$stmt->bind_param('ssss',$product_name,$product_desc,$product_area,$product_price);
